...
Submitted attachments remain in the thread history. Reverting a thread removes attachments belonging to discarded answers; deleting the thread uses the same file cleanup as other attachments. You can attach up to 20 files across one set of answers, using the same file and total image size limits as the normal composer.
