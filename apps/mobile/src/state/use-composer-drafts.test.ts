...
describe("mobile composer drafts", () => {
  ...
  it.each(["attachment", "context", "imported context"])(
    ...
    async (kind) => {
      ...
      expect(countComposerDraftAttachmentsAfterSelection(key, insertion)).toBe(19);
      expect(getComposerDraftAfterSelection(key, insertion).context?.records).toHaveLength(19);
      ...
      if (kind === "attachment") {
        ...
        expect(getComposerDraftSnapshot(key).attachments).toHaveLength(20);
      ...
      expect(draft.attachments.slice(0, 19)).toEqual(files.slice(1));
  ...
  it("releases videos rejected by the live draft limit and keeps accepted files", async () => {
    ...
    const existing = Array.from({ length: 19 }, (_, index) => makeAttachment(`held-${index}`));
    appAtomRegistry.set(composerDraftsAtom, {
    ...
    expect(draft?.attachments).toHaveLength(20);
    ...
    expect(appAtomRegistry.get(composerDraftsAtom)[draftKey]?.attachments).toHaveLength(21);
  });
  ...
  it("preserves existing images when shared content exceeds the draft attachment limit", () => {
    ...
    const sharedImages = Array.from({ length: 20 }, (_, index) => image(`shared-${index}`));
    ...
    expect(merged[draftKey]?.attachments).toHaveLength(20);
    expect(merged[draftKey]?.attachments[0]).toEqual(existingImage);
    expect(merged[draftKey]?.attachments.at(-1)?.id).toBe("shared-18");
  });
