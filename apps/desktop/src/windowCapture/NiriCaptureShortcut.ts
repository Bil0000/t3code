...
const PATH = "/com/t3tools/WindowCapture";
const INTERFACE = "com.t3tools.WindowCapture";

export function niriCaptureBinding(appId: string): string {
  return `Ctrl+Shift+2 repeat=false { spawn "gdbus" "call" "--session" "--dest" "${appId}.WindowCapture" "--object-path" "${PATH}" "--method" "${INTERFACE}.Capture"; }`;
}
...
export async function startNiriCaptureShortcut(
  ...
  try {
    const result = await Promise.race([
      bus.requestName(`${appId}.WindowCapture`, NameFlag.DO_NOT_QUEUE),
      failure,
