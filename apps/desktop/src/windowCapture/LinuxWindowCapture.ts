...
const EXTENSION = "org.gnome.Shell.Extensions.T3WindowCapture";
const EXTENSION_PATH = "/org/gnome/Shell/Extensions/T3WindowCapture";
...
export class LinuxCaptureConnection {
  ...
  async captureExtension(appId: string, options?: FeedbackOptions): Promise<LinuxWindowSnapshot> {
    const result = await this.wait(
      this.bus.requestName(`${appId}.WindowCapture`, NameFlag.DO_NOT_QUEUE),
    );
