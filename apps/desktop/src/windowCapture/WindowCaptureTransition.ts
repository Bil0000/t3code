...
export function windowCaptureAnimationDisplayBounds(
  displays: ReadonlyArray<Pick<Electron.Display, "bounds">>,
  source: Electron.Rectangle,
  destination: Electron.Rectangle,
): Array<Electron.Rectangle> {
  const flight = windowCaptureAnimationOverlayBounds([{ bounds: source }, { bounds: destination }]);
  return displays
...
export class WindowCaptureTransition {
  ...
  private async runFlight(
    ...
    if (this.boundOverlayToCaptureDisplays) {
      const missingBounds = windowCaptureAnimationDisplayBounds(
