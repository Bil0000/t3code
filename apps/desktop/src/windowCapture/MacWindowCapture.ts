...
export type MacWindowCaptureSource = {
...
function runMacWindowCapture(windowId: number, outputPath: string): Promise<void> {
  return new Promise((resolve, reject) => {
    NodeChildProcess.execFile(
      MAC_SCREEN_CAPTURE_PATH,
      macWindowCaptureArguments(windowId, outputPath),
...
): Promise<{ readonly source: MacWindowCaptureSource; readonly png: Buffer }> {
  await runMacWindowCapture(active.id, outputPath);
