export interface ProviderReplayGate {
  ...
  readonly waitForReached: (label: string) => Promise<void>;
...
export function makeProviderReplayGate(labels: ReadonlyArray<string>): ProviderReplayGate {
  ...
  return {
    ...
    waitForReached: (label) =>
      states.get(label)?.reachedPromise ??
      Promise.reject(new Error(`Unknown provider replay gate label ${label}.`)),
