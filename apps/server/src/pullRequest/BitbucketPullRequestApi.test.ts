...
layer("BitbucketPullRequestApi.layer", (it) => {
  ...
  it.effect("rewrites a title alone, and carries the reviewers back with it", () =>
    Effect.gen(function* () {
      mockedRequest
        .mockReturnValueOnce(
          Effect.succeed(
            response(pullRequestJson({ description: "The old body.", reviewers: [octocat] })),
          ),
        )
      ...
      // Reviewers left out of the body come off the pull request, so they travel back unchanged.
  ...
  it.effect("leaves out the half of the pull request it was not asked about", () =>
    Effect.gen(function* () {
      mockedRequest
        .mockReturnValueOnce(Effect.succeed(response(pullRequestJson({ description: "Old." }))))
        .mockReturnValueOnce(Effect.succeed(response("{}")));
      const api = yield* BitbucketPullRequestApi.BitbucketPullRequestApi;
  ...
  it.effect("writes both fields when both were rewritten", () =>
    Effect.gen(function* () {
      ...
      // @effect-diagnostics-next-line preferSchemaOverJson:off
      expect(JSON.parse(callAt(1).body ?? "")).toMatchObject({
        title: "A new title",
        description: "New body.",
      });
    }),
