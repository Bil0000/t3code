  ...
  describe("review diff previews", () => {
    it.effect("returns a truncated staged preview and refuses to unstage it", () =>
      Effect.gen(function* () {
        ...
        yield* writeTextFile(cwd, "large.txt", "later unstaged edit\n");
        const stagedFile = (yield* driver.getReviewDiffPreview({
          cwd,
          workingTreeScope: "staged",
          file: { path: "large.txt", previousPath: null, sourceKind: "staged" },
        })).sources[0]!;
        assert.isFalse(stagedFile.truncated);
        assert.include(stagedFile.diff, "+large staged line");
        assert.notInclude(stagedFile.diff, "later unstaged edit");
        const unstagedFile = (yield* driver.getReviewDiffPreview({
          cwd,
          workingTreeScope: "unstaged",
          file: { path: "large.txt", previousPath: null, sourceKind: "unstaged" },
        })).sources[0]!;
        assert.include(unstagedFile.diff, "-large staged line");
        assert.include(unstagedFile.diff, "+later unstaged edit");
        const error = yield* driver
          .applyReviewPatch({
            cwd,
            sourceKind: "staged",
