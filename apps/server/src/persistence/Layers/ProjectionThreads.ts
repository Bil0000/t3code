...
import { ModelSelection, ThreadLinkedPullRequest, ThreadPullRequestLink } from "@t3tools/contracts";
    ...
    execute: (row) =>
      sql`
        ...
        VALUES (
          ...
          ${JSON.stringify(
            row.pullRequestLinks ?? [
              ...(row.linkedPullRequest == null
                ? []
                : [{ ...row.linkedPullRequest, source: "linked" as const }]),
              ...(row.branchPullRequest == null
                ? []
                : [{ ...row.branchPullRequest, source: "branch" as const }]),
            ],
          )},
          ${row.latestTurnId},
          ${row.createdAt},
        ...
        DO UPDATE SET
          ...
          pull_request_links_json = excluded.pull_request_links_json,
